...
import { Home, Search, Target, Calendar, FileCheck, MessageSquare, Bell, User, Menu, ClipboardList, LogOut } from 'lucide-react';
...
export function IndustryLayout() {
  ...
  const navItems = [
    { path: '/industry', icon: Home, label: 'Dashboard' },
    { path: '/industry/marketplace', icon: Search, label: 'Marketplace' },
    { path: '/industry/problems', icon: ClipboardList, label: 'Problems' },
    { path: '/industry/smart-match', icon: Target, label: 'Smart Match' },
    { path: '/industry/meetings', icon: Calendar, label: 'Meetings' },
    { path: '/industry/licensing', icon: FileCheck, label: 'Licensing' },
    { path: '/industry/copilot', icon: MessageSquare, label: 'AI Copilot' },
  ];
      ...
      <aside className="sticky top-0 flex h-screen w-64 flex-col border-r border-neutral-200/80 bg-white/95 backdrop-blur-sm shadow-[1px_0_0_rgba(16,24,40,0.02),8px_0_24px_-16px_rgba(16,24,40,0.08)]">
        <div className="flex items-center gap-3 px-5 py-5 border-b border-neutral-100">
          ...
          <div>
            ...
            <div className="text-[0.7rem] font-medium uppercase tracking-wider text-neutral-400">Industry Portal</div>
          </div>
        </div>
