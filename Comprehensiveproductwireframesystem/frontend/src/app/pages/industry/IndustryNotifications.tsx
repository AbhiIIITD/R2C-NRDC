...
import { Bell, Calendar, FileCheck, Star } from 'lucide-react';

const iconForType = (type: string) => {
  if (type.includes('meeting')) return Calendar;
  if (type.includes('license')) return FileCheck;
  if (type.includes('published')) return Star;
  return Bell;
};
...
export function IndustryNotifications() {
  ...
  const openNotification = (notificationId: string, relatedId?: string, relatedType?: string) => {
    ...
    if (relatedType === 'study' && relatedId) navigate(`/industry/technology/${relatedId}`);
    else if (relatedType === 'meeting') navigate('/industry/meetings');
    else if (relatedType === 'license') navigate('/industry/licensing');
    else navigate('/industry/marketplace');
  };
    ...
    <div>
      <div className="mb-6 flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-neutral-600">Stay updated on technology opportunities</p>
        </div>
      ...
      <div className="space-y-3">
        {notifications.length === 0 ? (
          <div className="border-2 border-neutral-400 bg-white p-12 text-center">
            ...
            <div className="text-sm text-neutral-600">Smart matches, meeting approvals, and license updates will appear here.</div>
          </div>
        ) : (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="flex gap-2">
                    ...
                    />
                    {!notification.read && (
                      <WireframeButton label="Dismiss" variant="ghost" size="sm" onClick={() => markNotificationAsRead(notification.id)} />
                    )}
                  </div>
                </div>
              </div>
